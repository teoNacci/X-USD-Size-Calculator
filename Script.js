...
// opening and closing behavior for all windows.
// calculator-input1
document.addEventListener("click", a => {
    const isriskinput = a.target.matches("[data-riskinput]");
    const isopenbutton = a.target.matches("[data-openbutton]");
    const isclosebutton = a.target.matches("[data-closebutton]");
    if (isopenbutton) {
        if(active1 == 0){
            currentinput = a.target.closest('[data-calculatorinput1]');
            document.querySelector('.calculator-input1.active')?.classList.remove('active');
            document.querySelector('.calculator-input2.active')?.classList.remove('active');
            document.querySelector('.calculator-input3.active')?.classList.remove('active');
...
document.addEventListener("click", b => {
    const ispricevarinput = b.target.matches("[data-pricevarinput]");
    const isopenbutton2 = b.target.matches("[data-openbutton2]");
    const isclosebutton2 = b.target.matches("[data-closebutton2]");
    if (isopenbutton2) {
        if(active2 == 0){
            currentinput = b.target.closest('[data-calculatorinput2]');
            document.querySelector('.calculator-input1.active')?.classList.remove('active');
            document.querySelector('.calculator-input2.active')?.classList.remove('active');
            document.querySelector('.calculator-input3.active')?.classList.remove('active');
            document.querySelector('.container.active')?.classList.remove('active');
...
document.addEventListener("click", c => {
    const islotsizeinput = c.target.matches("[data-lotsizeinput]");
    const isopenbutton3 = c.target.matches("[data-openbutton3]");
    const isclosebutton3 = c.target.matches("[data-closebutton3]");
    if (isopenbutton3) {
        if(active3 == 0){
            currentinput = c.target.closest('[data-calculatorinput3]');
            document.querySelector('.calculator-input1.active')?.classList.remove('active');
            document.querySelector('.calculator-input2.active')?.classList.remove('active');
            document.querySelector('.calculator-input3.active')?.classList.remove('active');
            document.querySelector('.container.active')?.classList.remove('active');
...
// calculator-input3 --> info
document.addEventListener("click", d => {
    const isinfo = d.target.matches("[data-infobutton]");
    if (!isinfo && d.target.closest('[data-infosize]')!= null) return;
    if (isinfo) {
        if(active4 == 1){
            currentinput = d.target.closest('[data-infosize]');
...
// calculator-body --> switch_ Normal/Pro
document.addEventListener("click", f => {
    const isSwitch = f.target.matches("[data-switch]");
    const isHandle = f.target.matches("[data-handle]");
    if (isSwitch || isHandle) {
        if (active8 == 0){
            document.querySelector('.box').classList.add('active');
            document.querySelector('.open-button').classList.add('active');
            document.querySelector('.open-button2').classList.add('active');
            document.querySelector('.open-button3').classList.add('active');
            active8 = 1;
            switchState = 1;
            return;
        }
        ...
        if (active8 == 1){
            ...
            document.querySelector('.open-button2').classList.remove('active');
            document.querySelector('.open-button3').classList.remove('active');
            active8 = 0;
            switchState = 0;
            return;
        }
...
// calculator-input2 --> sub-inputs: on-input check user interaction
// and set limitation based on what pair he's interacting with. 
document.querySelectorAll('input[id="npips"]').forEach(input =>{
    input.oninput = () => {
...
document.querySelectorAll('input[id="pipdim"]').forEach(input =>{
    input.oninput = () => {
        if (entryprice.value > 0 || exitprice.value > 0){
            pipdim.value = "";
        } else {
...
document.querySelectorAll('input[id="entryprice"]').forEach(input =>{
    input.oninput = () => {
...
document.querySelectorAll('input[id="exitprice"]').forEach(input =>{
    input.oninput = () => {
        if (npips.value > 0 || pipdim.value > 0){
            exitprice.value = "";
        } else {
    ...
    if (risk.value == 0 || pricevar.value == 0 || size.value == 0){
        ...
        display2.value = "";
            ...
            function Add(){
                ...
                z += y;
                z1 += y1;
                display.value = z.toFixed(fix5);
                display2.value = z1.toFixed(fix4);
                if (fCount == nIter){
                    i = ((risk.value / pricevar.value) / size.value);
                    j = (risk.value / pricevar.value);
                    display.value = i.toFixed(fix5);
                    display2.value = j.toFixed(fix4);
...
function Reset(){
    ...
    pricevar.value = "";
    size.value = "";
    document.querySelector('.npips-input').classList.remove('active');
    document.querySelector('.pip-value-input').classList.remove('active');
    document.querySelector('.entryprice-input').classList.remove('active');
    document.querySelector('.exitprice-input').classList.remove('active');
    active6 = 0;
    active7 = 0;
}
...
function BlurPipsByDrop(){
    document.querySelector('.entryprice-input').classList.remove('active');
    document.querySelector('.exitprice-input').classList.remove('active');
    document.querySelector('.npips-input').classList.add('active');
    document.querySelector('.pip-value-input').classList.add('active');
}

function BlurPipsByClick(){
    if (active6 == 1){
        return;
    } else {
        document.querySelector('.entryprice-input').classList.remove('active');
...
function BlurPriceByDrop(){
    document.querySelector('.npips-input').classList.remove('active');
    document.querySelector('.pip-value-input').classList.remove('active');
    document.querySelector('.entryprice-input').classList.add('active');
    document.querySelector('.exitprice-input').classList.add('active');
}

function BlurPriceByClick(){
    if (active7 == 1){
        return;
    } else {
        document.querySelector('.npips-input').classList.remove('active');
...
function CheckPips(){
    if (npips.value > 0 || pipdim.value > 0){
        document.querySelector('.entryprice-input').classList.add('active');
        document.querySelector('.exitprice-input').classList.add('active');
    } else if (npips.value == 0 || pipdim.value == 0){
        document.querySelector('.entryprice-input').classList.remove('active');
        document.querySelector('.exitprice-input').classList.remove('active');
        active7 = 0;
    }
